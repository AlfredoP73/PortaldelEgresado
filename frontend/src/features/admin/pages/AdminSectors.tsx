...
import api from '../../../api';
import { Plus, Trash2, Loader2, Save, Edit2, X, Tags, Download, Upload } from 'lucide-react';
...
export default function AdminSectors() {
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(dataToExport, 'Sectores');
  };
    ...
    try {
      ...
      for (const row of data) {
        try {
          ...
          errorCount++;
          console.error('Error importando sector:', row, err);
        }
      }
  ...
  return (
    <div className="space-y-6">
      <div className="page-header flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="page-title text-[var(--text-main)]">{t('sectors.title')}</h2>
          <p className="text-sm mt-1 text-[var(--text-ink-secondary)]">{t('sectors.subtitle')}</p>
        </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2">
              <Plus className="w-4 h-4" /> {t('sectors.new')}
            </button>
          </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        {loading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center justify-center">
            <Tags className="w-12 h-12 mb-4 opacity-50" style={{ color: 'var(--text-ink-tertiary)' }} />
            <h3 className="text-lg font-bold" style={{ color: 'var(--text-main)' }}>{t('sectors.empty_title')}</h3>
            <p className="mt-1" style={{ color: 'var(--text-ink-secondary)' }}>{t('sectors.empty_desc')}</p>
          </div>
        ) : (
          <>
            <table className="w-full text-left text-sm">
              <thead className="bg-[var(--bg-muted)] border-b border-[var(--border-color)]">
                <tr>
                  ...
                  <th className="px-6 py-4 w-full font-bold uppercase text-[11px] tracking-wider" style={{ color: 'var(--text-ink-secondary)' }}>{t('sectors.col_name')}</th>
      ...
      <Modal isOpen={showModal} onClose={closeModal} maxWidth="max-w-md">
        <div className="flex justify-between items-center p-6 border-b shrink-0" style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-surface)' }}>
          <h3 className="text-lg font-bold font-heading" style={{ color: 'var(--text-main)' }}>
            {editingItem ? t('sectors.modal_edit') : t('sectors.modal_new')}
          </h3>
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5 overflow-y-auto flex-1">
          <div>
            <label className="form-label">{t('sectors.input_label')}</label>
            <input
              type="text"
              value={newItemName}
              onChange={(e) => setNewItemName(e.target.value)}
              className="input w-full"
              placeholder={t('sectors.input_placeholder')}
              autoFocus
              required
            />
          </div>
